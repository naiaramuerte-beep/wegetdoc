...
57. **FastPay orphan charges: reconciliación automática + recuperación manual desde admin.** Tras recuperar manualmente a la cliente `huzinafranciska@gmail.com` (userId 53376, Sipay txn 174355749) que pagó 0,50€ pero nunca tuvo subscripción porque el redirect `/api/sipay/callback/ok` no llegó, montado el sistema para que no vuelva a pasar:
    - **Causa raíz**: el callback de FastPay depende 100% de que el navegador del usuario complete el redirect desde Redsys MPI después de 3DS. Si cierra la pestaña, el Wi-Fi falla, el iframe se rompe, o Redsys retrasa la redirección, Sipay cobra y nosotros nos quedamos sin webhook server-to-server (Sipay no los manda) — DB sin sub, customer pagó, admin no se entera. Solo `fastpay_init_started` quedó loggeado para huzinafranciska, ningún `fastpay_intro_charge`.
    - **Helper idempotente `finalizeFastpayPayment({ requestId, fallbackUserId?, source })`** en `server/_core/sipay.ts:155`: extrae toda la lógica que antes vivía inline en el callback (`confirmPayment` → leer payload → `upsertSubscription` + `markDocumentsPaid` + `recordCharge` + email de bienvenida). Chequea `findIntroChargeForRequest()` antes de escribir nada → seguro de re-ejecutar sin duplicar suscripciones ni charges. Lee `custom_01` del payload (lo metimos en `sipayCheckoutInit`) para correlacionar el txn con el userId, con `fallbackUserId` como red de seguridad para recuperaciones manuales.
    - **Reconciliación cron `POST /api/cron/sipay-finalize-pending`** (Express en `_core/index.ts`, autenticado con `X-Cron-Secret`, `?dry=1` para validar sin escribir): escanea `webhook_events` por eventos `fastpay_3ds_pending` de las últimas 24h sin un `fastpay_intro_charge` que matchee, y corre `finalizeFastpayPayment` en cada uno. Devuelve outcomes por requestId. **Hay que registrar este endpoint en Railway scheduled tasks con frecuencia 5-15 min** — `curl -X POST ... -H "X-Cron-Secret: $CRON_SECRET"`.
    - **Nuevo evento `fastpay_3ds_pending`** loggeado en `sipayCheckoutInit` mutation justo después de que Sipay devuelve el `request_id` y la URL de 3DS. El payload incluye `{ userId, order, requestId, amountCents }` para que el cron tenga todo lo necesario sin tener que re-leer la sesión. Es el "marcador de huérfana potencial".
    - **Helpers nuevos en `server/db.ts`**: `findIntroChargeForRequest({ order, sipayTxn, requestId })` (probe de idempotencia) y `findPendingFastpayPayments({ hoursBack, limit })` (lista de huérfanas pendientes que el cron y el admin consumen).
    - **Admin UI: pestaña Facturación, sección "Pagos huérfanos"**: tabla nueva en la parte superior de la pestaña que muestra los pagos pendientes en vivo (refetch cada 30s). Cada fila tiene un botón rojo "Reconciliar" que dispara la mutation `admin.recoverOrphanPayment({ requestId, userId })` (audit-logged). El backend corre el mismo `finalizeFastpayPayment` con `source: "admin"`. Cuando ya estaba reconciliado, el toast muestra "Ya estaba reconciliado — no se ha duplicado nada." en lugar del éxito normal.
    - **Admin UI: Suscriptores ahora muestra huérfanas recuperadas**: `getAllSubscribedUsers()` en `db.ts:198` ya no filtra por `sipayToken IS NOT NULL` (antes excluía huérfanas), ahora filtra por `status IN ('trialing', 'active', 'past_due')`. Si la fila no tiene `sipayToken`, se renderiza un badge rojo `⚠ sin renovación` para que el admin sepa que esa sub no se va a renovar (no podemos cobrarle MIT-R sin token). El cron de renovación `getSubsDueForRenewal()` mantiene su filtro estricto — solo itera filas que SÍ se pueden cobrar.

56. **Google Pay desactivado temporalmente — `OR_BIBED_11` el día del go-live.** Google aprobó `www.editorpdf.net` para producción el 2026-06-29 (email "cleared for launch" del Google Pay API Team). Pero en cuanto el user intentó la primera transacción real, el sheet de Google devuelve `OR_BIBED_11 — este comercio no puede aceptar tu pago`. El error es de Google rechazando al merchant antes de tokenizar — NO llega ni a `chargeGpay` en backend.
    - **Causa raíz más probable (pendiente de confirmar con Sipay)**: el `gatewayMerchantId` que enviamos al `PaymentDataRequest` de Google es nuestro `ENV.sipayKey` (lo que llega via `sipayConfig` tRPC procedure). Sipay típicamente registra UN `gatewayMerchantId` específico con Google para la integración GPay, que NO coincide con la `SIPAY_KEY` general de FastPay. Sin ese ID correcto Google rechaza al merchant. Pinged Sipay support con el contexto exacto (token con GPay activado).
    - **Mitigación**: nueva constante `GPAY_ENABLED` en `PaywallModal.tsx` flippeada de `true` a `false`. El botón GPay no se renderiza, los usuarios ven solo Apple Pay (Safari/iOS) + Tarjeta (FastPay iframe), ambos siguen cobrando normalmente. El componente `<GooglePayButton>` queda montado tras el flag para no perder integración cuando Sipay responda con el ID correcto.
    - **Para reactivar cuando Sipay responda**: flippear `GPAY_ENABLED = true` + reemplazar `gatewayMerchantId: sipayMerchantKey` por el ID dedicado que Sipay envíe + push. Si el ID es global, hardcodearlo. Si es por-comercio (caso normal), pasarlo via `sipayConfig` como `gpayGatewayMerchantId` adicional, separado de `key`.

55. **Lazy-chunk recovery: auto-reload on stale-bundle errors después de un deploy.** Sentry issue `7be6998900e9` ("TypeError: Failed to fetch dynamically imported module: .../assets/EditorPage-BFgy59ZX.js" en `/uk` desde Opera) era la firma clásica de tab vieja después de deploy: el cliente tenía un `index.html` que referenciaba un chunk con hash viejo que Vite ya borró al generar el nuevo bundle. Cuando el usuario navegaba a la ruta que disparaba `lazy(() => import("./pages/EditorPage"))` → 404 → unhandled rejection.
    - **Nuevo `client/src/lib/chunkReload.ts`**: helper `installChunkErrorRecovery()` instala 3 listeners globales (`vite:preloadError` que emite el helper de Vite, `unhandledrejection` para la rejection del `import()` que cae al React.lazy boundary, y `error` para casos de script-tag clásico). Cada uno hace match contra patrones conocidos ("Failed to fetch dynamically imported module", "Importing a module script failed", "Loading chunk", "Loading CSS chunk", "error loading dynamically imported module") y dispara `window.location.reload()` exactamente una vez. Guarda timestamp en `sessionStorage["vite-chunk-reload-ts"]` y si pasó <30s desde el último reload, no recarga (evita loops infinitos si el bundle nuevo también falla — CDN propagation lag, outage, etc.). El reload trae el `index.html` fresco con los hashes nuevos y el usuario sigue.
    - **`client/src/main.tsx`**: añadido `installChunkErrorRecovery()` justo después de `initSentry()` y ANTES de cualquier import de `App` (que es el que arrastra los 25 `lazy()`). Los listeners están en sitio antes de que React monte la primera ruta.
    - **`client/src/lib/sentry.ts`**: añadidos los 5 patrones de chunk-error a `IGNORED_ERRORS`. Estos errores ya los recuperamos auto-magicamente con el reload, así que reportarlos en Sentry sólo gasta cuota — el usuario nunca los ve.
    - **Por qué hace falta el handler tanto en `vite:preloadError` como en `unhandledrejection`**: Vite emite `vite:preloadError` cuando un `<link rel="modulepreload">` falla (preload del chunk antes del import). Pero si el preload no se hizo o falló silenciosamente, el error que vemos es la rejection del `import()` que React.lazy mete en una Promise. Sin el segundo listener no cubrimos el caso del Sentry issue real.

54. **Paywall: cards "Tu archivo por solo" + GPay con paleta azul Sipay (ya no negro).** El user quería que los fondos negros del modal de pago coincidieran con el azul del botón "Descargar" del iframe FastPay de Sipay. Cambios:
    - **Gradient `#0A0A0B → #1A1A1C`** → `#1E66C9 → #1551A8` aplicado a 3 ubicaciones via `replace_all`: el card de precios "Tu archivo por solo / 0,50 €" (paso de pago) + el card del trial-limit + cualquier otro uso del gradient.
    - **Google Pay button**: cambiado de `buttonColor: "black"` a `buttonColor: "white"` (Google solo permite `default | black | white` por brand guidelines — `white` se ve menos pesado al lado del nuevo card azul).
    - Las copias `paywall_your_pdf` ya estaban renombradas de "Tu PDF por solo" → "Tu archivo por solo" en este commit; ahora el visual concuerda.

53. **i18n: "Tu PDF por solo" → "Tu archivo por solo" en 12 idiomas.** La frase del card de precios del paywall usaba "PDF" cuando el archivo a descargar puede ser .docx / .xlsx / .pptx / .jpg (en el flujo del converter). Cambiado a "archivo / file / fichier / Datei / ficheiro / file / bestand / plik / файл / файл / fișier / 文件" en `paywall_your_pdf` × 12 langs.
