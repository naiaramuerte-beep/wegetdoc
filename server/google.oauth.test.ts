...
describe("Google OAuth credentials", () => {
  ...
  it("should build a valid Google auth URL", () => {
    const clientId = process.env.GOOGLE_CLIENT_ID ?? "";
    const redirectUri = "https://wegetdoc.com/api/auth/google/callback";
    const state = Buffer.from(JSON.stringify({ origin: "https://wegetdoc.com", returnPath: "/" })).toString("base64url");
    ...
    expect(url).toContain(encodeURIComponent("wegetdoc.com"));
  });
});
