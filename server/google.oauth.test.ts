...
describe("Google OAuth credentials", () => {
  ...
  it("should build a valid Google auth URL", () => {
    const clientId = process.env.GOOGLE_CLIENT_ID ?? "";
    const redirectUri = "https://editorpdf.net/api/auth/google/callback";
    const state = Buffer.from(JSON.stringify({ origin: "https://editorpdf.net", returnPath: "/" })).toString("base64url");
    ...
    expect(url).toContain(encodeURIComponent("editorpdf.net"));
  });
});
