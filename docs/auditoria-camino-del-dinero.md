...
## Actualización 2026-08-11 — el candado se desplegó, se revirtió y hay que repetirlo

El candado se fusionó a `main` el **2026-08-09 09:16 UTC** (11:16 Madrid) y se
revirtió **45 minutos después**, a las 10:01 UTC (12:01 Madrid), sin que hubiera
ningún indicio de avería.

Forense de esa ventana (`scripts/forense-revert-9ago.mjs`, sobre producción):

- **Cero tráfico mientras el candado estuvo vivo.** Ni un solo
  `*_init_started`, ni un cargo, en los 45 minutos. Era domingo por la mañana.
- **El candado no llegó a bloquear nada nunca:** 0 eventos
  `alta_duplicate_blocked` y 0 `alta_price_mismatch` en todo el histórico.
- Los dos únicos errores de la franja fueron `gpay_charge_failed`
  (`authorization_error`) a las 08:21 y 08:24 UTC — **antes** del despliegue, y
  del problema conocido de los wallets, no del candado.

Es decir: no se revirtió por evidencia en contra, se revirtió por no tener
evidencia a favor. La causa de fondo fue la alarma de la primera versión de
`scripts/vigilancia-candado.mjs`, que comparaba las altas de un domingo contra
una media de 7 días contaminada por el viernes (×17 de volumen) y gritaba
"POSIBLE BLOQUEO TOTAL" cuando lo que faltaba era tráfico. Esa alarma ya está
corregida (commit `7ed93fd`, que sigue en `main`): ahora exige que HAYA HABIDO
intentos antes de hablar de bloqueo.

**Coste del rollback:** el fallo volvió a morder al día siguiente. El usuario
108054 (`rubilnikinfo@gmail.com`) pagó dos veces el 2026-08-10 a las 12:42 y
12:45 UTC (cargos #1073 y #1074, 3 minutos de separación). La lista de
reembolsos pasa de 24 cargos / 12,00 € a **25 cargos / 12,50 €**.

**Qué falta para volver a desplegar:** un despliegue en día con tráfico y el
smoke de verdad — ver el primer `alta_duplicate_blocked` en
`scripts/vigilancia-candado.mjs`. El precio no cambia: `intro_price_eur` no
existe en `site_settings`, así que `getIntroPriceCents()` devuelve el defecto de
50 c, exactamente lo que el cliente ya manda hoy (`amountCents: 50` en
`PaywallModal.tsx`). No debe aparecer ni un `alta_price_mismatch`.
